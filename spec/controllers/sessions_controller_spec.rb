  ...
  describe "Logging in" do
    ...
    describe "by password" do
      before(:each) do
        ...
        User.stub!(:authenticate_by_password).with(anything(), anything()).and_return(@user)
      ...
      describe "successfully" do
        ...
        it "asks to authenticate me"                       do User.should_receive(:authenticate_by_password).with('test_login', 'monkey'); do_login end
        # cookies
      ...
      handled_errors.each do |error_type, error_outcome, error_dest, error_msg|
        describe "on failed login because of #{error_type.to_s.underscore.humanize}" do
          before(:each) do
            User.stub!(:authenticate_by_password).with(anything(), anything()).and_raise(error_type.new)
          end
      ...
      describe "on failed login because of an error not related to security" do
        before(:each) do
          User.stub!(:authenticate_by_password).with(anything(), anything()).and_raise(Exception)
        end
