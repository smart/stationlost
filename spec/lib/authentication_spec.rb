...
describe Authentication do
  ...
  describe "logged_in?" do
    ...
    it 'is true  for current_user set'   do @mock_controller.current_user = @user; @mock_controller.logged_in?.should be_true  end
  ...
  describe "current_user accessor" do
    ...
    it "returns @current_user when it's set" do
      @mock_controller.current_user = @user;
      @mock_controller.should_not_receive(:try_login_from_session)
      @mock_controller.should_not_receive(:try_login_chain)
  ...
  describe "become_logged_in_as!" do
    ...
    it "raises an AuthenticationError unless user" do
    ...
    it "raises the given error if authorization fails" do
      ...
      lambda{ b_l_i_a! @user }.should raise_error(my_exception)
    end
    it "sets via the current_user setter"           do stub_auth!(@mock_controller, true); @mock_controller.should_receive(:current_user=).with(@user); b_l_i_a!(@user) end
    it "returns current_user if it can log in"      do stub_auth!(@mock_controller, true); b_l_i_a!(@user).should == @user  end
  end
  describe "become_logged_in_as!" do
    def b_l_i_a_no_raise(u) @mock_controller.send(:become_logged_in_as, u) end
    it "asks for authorization" do
      @mock_controller.should_receive(:get_authorization).at_least(:once).with({:for => @user, :to => :login, :on => nil, :context => nil}).and_return(true)
      b_l_i_a_no_raise @user
    end
    it "raises an AuthenticationError unless user"  do
    ...
    it "sets current_user=false if not authorized"  do stub_auth!(@mock_controller, false); @mock_controller.should_receive(:current_user=).with(false); b_l_i_a_no_raise(@user) end
    it "returns false (not nil) if not authorized"  do stub_auth!(@mock_controller, false); b_l_i_a_no_raise(@user).should be_false    end
    it "sets via the current_user setter"           do stub_auth!(@mock_controller, true);  @mock_controller.should_receive(:current_user=).with(@user); b_l_i_a_no_raise(@user) end
    it "returns current_user if it can log in"          do stub_auth!(@mock_controller, true);  b_l_i_a_no_raise(@user).should == @user   end
  end
  ...
  describe "Logging in from session" do
    ...
    it "Finds user if session user_id is set" do
      ...
      User.should_receive(:find_by_id).with('fake_user_id').and_return(@user)
      @mock_controller.should_receive(:current_user=).with(@user)
      @mock_controller.try_login_from_session.should == @user
    end
